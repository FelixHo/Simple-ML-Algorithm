...
class LogisticRegression(object):
    ...
    def _cal_error(self, W, X, y):
        ...
        return np.sum(np.log(1 + np.exp(z)) - y * z + self.lam * W ** 2 / 2) / X.shape[0]
    ...
    def predict(self, X_test):
        ...
        pred_true = self._logistic_hypothesis(self.W, X)
        prob = np.concatenate([1 - pred_true, pred_true], axis=1)
